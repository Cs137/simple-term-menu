    ...
    class Selection:
        def __init__(
            ...
            self._selected_menu_indices = (
                set(preselected_indices)
                if preselected_indices is not None
                else {}
                if unsorted_multi
                else set()
            )
            self._unsorted_multi = unsorted_multi
        ...
        def __setitem__(self, menu_index: int, is_selected: bool) -> None:
            if is_selected:
                if isinstance(self._selected_menu_indices, dict):
                    self._selected_menu_indices.update({menu_index: None})
                else:
                    self._selected_menu_indices.add(menu_index)
            else:
                if isinstance(self._selected_menu_indices, dict):
                    self._selected_menu_indices.pop(menu_index)
                else:
        ...
        def selected_menu_indices(self) -> Tuple[int, ...]:
            if isinstance(self._selected_menu_indices, dict):
                return tuple(self._selected_menu_indices)
            else:
    ...
    def __init__(
        ...
        *,
        ...
        self._multi_select_keys = (
            tuple(multi_select_keys)
            if multi_select_keys is not None
            else []
            if unsorted_multi
            else ()
        )
        ...
        self._unsorted_multi = unsorted_multi
        # if unsorted_multi and not self._multi_select:
        #     raise InvalidParameterCombinationError(
        #         "Multi-select mode must be enabled when unsoerted indices are requested."
        #     )
        ...
        self._selection = self.Selection(
            len(self._menu_entries), self._unsorted_multi, self._preselected_indices
        )
...
""",
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="unsorted_multi",
        help="returns unsorted menu_entry_indices if True (order of selection)",
    )
    return parser
...
def main() -> None:
    ...
    try:
        terminal_menu = TerminalMenu(
            ...
            title=args.title,
            unsorted_multi=args.unsorted_multi,
        )
    except (
